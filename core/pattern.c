...
static float layer_score_raw(int x, int y) {
    Cell c = canvas_get(x, y);
    return c.energy / 255.0f;
}

static float layer_score_edge(int x, int y, int radius) {
    ...
    return (diff / cnt) / 255.0f;
}

static float layer_score_shape(int x, int y, int radius) {
    ...
    return total > 0 ? (float)active / total : 0.0f;
}

static float layer_score_object(int x, int y, int radius) {
    float sum_id = 0.0f;
    int   cnt    = 0;
    for (int dy = -radius; dy <= radius; dy++) {
        for (int dx = -radius; dx <= radius; dx++) {
    ...
    if (cnt == 0) return 0.0f;
    return fminf(1.0f, (sum_id / cnt) / 65535.0f);
}

static float layer_score_context(int x, int y, int radius) {
    return layer_score_shape(x, y, radius * 2 > 10 ? 10 : radius * 2) * 0.5f +
           layer_score_object(x, y, radius) * 0.5f;
}

static float layer_score_abstract(int x, int y, int radius) {
    return (layer_score_raw(x, y) + layer_score_edge(x, y, radius) +
            layer_score_shape(x, y, radius)) / 3.0f;
}
...
void pattern_train(int x, int y, PatternLayer layer, uint16_t id) {
    ...
    trained[trained_count].confidence = 1.0f;
    trained_count++;
}
